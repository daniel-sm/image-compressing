...
def compute_frequencies_rgb(image: np.ndarray) -> FREQUENCIES_TYPE:
...
def encode_huffman_rgb(image: IMAGE_BYTE) -> IMAGE_BYTE:
    freqs = compute_frequencies_rgb(image)
    codes = build_huffman_tree_rgb(freqs)
...
def decode_huffman_rgb(encoded: IMAGE_BYTE) -> IMAGE_BYTE:
...
def main():
    ...
    encoded = encode_huffman_rgb(image)
    print("Encoded:", encoded.dtype, encoded.shape, encoded.size)

    decoded = decode_huffman_rgb(encoded)
